...
def _plot_convergence_curves(
    ...
    ----------
    history : list
        List of snapshot dicts produced every log_step epochs during training.
    output_dir : pathlib.Path
        Directory where PNGs are saved.
    bounds : dict
        Deviation bounds used during training, keyed by parameter group name.
    """
    if not history:
        return
...
def _plot_parameter_histograms(
    ...
    One sub-directory ``param_histograms/`` is created with four PNGs per heliostat group:
    ...
    Each panel shows the distribution of final values across all N heliostats in the group,
    vertical dashed lines at ±bound, and a saturation annotation (% of heliostats at bound).
    """
    hist_dir = output_dir / "param_histograms"
    hist_dir.mkdir(parents=True, exist_ok=True)

    def _saturation_pct(values_np: np.ndarray, bound: float) -> float:
        """Fraction of values (in %) whose absolute value is ≥ 99 % of the bound."""
