"""Generic per-heliostat version of `be25_occlusion_sweep.py` (formerly BE25-only).

Uses the same generic real-data pipeline as `geometry_schematic.py` /
...
the studied heliostat's real neighbours.

The studied heliostat is aimed at the target's centre (synthetic forward-aim
-- no trained kinematic model needed for this geometric sweep). The
identified blockers are swept continuously from VERTICAL (tilt=0, max
blocking) to HORIZONTAL (tilt=1, no blocking).

Four outputs, written to
outputs/new_mapping_function/blocking_study/blocking_validation/<heliostat_id>/:
  1. plots/centroid_trail_over_flux.png
  2. plots/centroid_trajectory_meters.png
  3. gifs/focal_spot_occlusion_sweep.gif
  4. gifs/focal_spot_occlusion_sweep_with_centroid.gif

Usage
-----
    python occlusion_sweep.py --heliostat-id BE25 --sun-sample 0027
    python occlusion_sweep.py --heliostat-id BH70 --sun-sample 0011 --n-steps 20 --fps 2
"""
...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", default=None,
                         help="Stage-1 checkpoint .pt to apply instead of ideal nominal kinematics "
                              "(see blocking_utils.apply_stage1_checkpoint). Also renames the output "
                              "folder to <heliostat_id>_trained so it doesn't clobber the ideal run.")
    ...
    if not blocker_rows:
        raise RuntimeError(f"{heliostat_id}: no blockers identified for sample {args.sun_sample}")
