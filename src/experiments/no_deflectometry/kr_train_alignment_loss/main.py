...
_experiments = _no_defl.parent         # .../src/experiments/
...
IS_ON_DAIC = True
SMOKE_TEST = not IS_ON_DAIC  # runs locally with 1 heliostat, 3 epochs
...
from artist_extensions.kinematic_reconstructors import WortbergAlignmentReconstructor
...
OUTPUT_DIR = (
    BASE_DIR / "outputs" / "local_runs" / "no_deflectometry" / f"alignment_kr_{_run_timestamp}"
    if not IS_ON_DAIC
    else BASE_DIR / "outputs" / "no_deflectometry" / f"alignment_kr_{_run_timestamp}"
)
