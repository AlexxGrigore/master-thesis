...
_pkg = pathlib.Path(__file__).parent   # .../src/kr_annealing_pixel_loss/
_experiments = _pkg.parent             # .../src/experiments/old_code/
_src = _experiments.parent             # .../src/experiments/
_src = _src.parent                     # .../src/
sys.path.insert(0, str(_src))
sys.path.insert(0, str(_experiments.parent))
...
OUTPUT_DIR = (
    BASE_DIR / "outputs" / "local_runs" / f"annealing_kr_{_run_timestamp}"
    if not IS_ON_DAIC
    else BASE_DIR / "outputs" / f"annealing_kr_{_run_timestamp}"
)
...
# ===================================================================
#
# Single-phase annealing training:
#
# The loss linearly interpolates from FocalSpotLoss (epoch 0) to
# PixelLoss (epoch max_epoch).  Both are normalised by their initial
# value so they contribute equally at the midpoint.
#
# lr=1e-3 matches what worked for focal-spot-only training.
# max_epoch=300 gives the full annealing trajectory.

LOSS_NAME = "annealing_pixel_loss"
...
try:
    with setup_distributed_environment(
        ...
        print(f"\n{'=' * 60}")
        print(f"EXPERIMENT: {LOSS_NAME} (annealing)")
        print("=" * 60)
        print(f"  FocalSpotLoss → PixelLoss annealing")
        print(f"  lr={OPT_CONFIG[config_dictionary.initial_learning_rate]}, "
